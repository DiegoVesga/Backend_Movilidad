...
class ruta_lugar_estrategico(db.Model):
    ...
    id_lugar_ruta = db.Column(db.Integer, primary_key = True)
    id_lugar = db.Column(db.Integer, db.ForeignKey('tbllugar_estrategico.id_lugar'))
    id_ruta = db.Column(db.Integer, db.ForeignKey('tblruta.id_ruta'))

    def __init__(self,id_lugar_ruta,  id_lugar, id_ruta):
        self.id_lugar_ruta = id_lugar_ruta
...
class ruta_lugar_estrategicoSchema(ma.Schema):
    class Meta:
        fields = ('id_lugar_ruta','id_lugar', 'id_ruta')
